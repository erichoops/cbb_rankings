...
#     df_filtered
    ...
    df['combined_avg'] = round(((df['resume_metric_avg'].astype(float)/(1/resume_weight) + df['predictive_metric_avg'].astype(float)/(1/predictive_weight))),1)
    ...
    cols = ['rank', 'team', 'resume_metric_avg', 'predictive_metric_avg', 'combined_avg'] #'recency_rank'
    df = df[cols]
